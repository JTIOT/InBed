    ...
    public partial class UserService : ServiceBase<UserEntity>, IDependency, IUserService
    {
		...
		public bool Add(UserDto dto)
		{
            using (var scope = _dbScopeFactory.Create())
            {
                ...
                int userID = Convert.ToInt32(((FormsIdentity)(HttpContext.Current.User.Identity)).Ticket.UserData);

                var onlineUser = GetOne(p => p.Id == userID);

                dto.FacilitatorCode = onlineUser.FacilitatorCode;
                dto.FacilitatorName = onlineUser.FacilitatorName;
                dto.Creater = onlineUser.Id;
                var db = GetDb(scope);
