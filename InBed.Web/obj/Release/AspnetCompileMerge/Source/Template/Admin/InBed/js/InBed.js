...
InBed = {};
//返回上一页
InBed.goback = function(e) {
    $(e).button("loading").delay(1000).queue(function() {
        history.go(-1);
    });
}
//跳转到指定的页面
InBed.goAction = function (e, moudleId, menuId, btnId, url, grid) {
    var addReg = /add/gi;
    var editReg = /edit/gi;
    ...
    url = url.toLowerCase();
    if (addReg.test(url)) {
        InBed.addModel(e, moudleId, menuId, btnId, url, grid);
    } else if (editReg.test(url)) {
        InBed.editModel(e, moudleId, menuId, btnId, url, grid);
    } else if (deleteReg.test(url)) {
        InBed.delModel(e, moudleId, menuId, btnId, url, grid);
    } else if (authen.test(url)) {
        InBed.authen(e, moudleId, menuId, btnId, url, grid);
    }
}
//addModel
InBed.addModel = function (e, moudleId, menuId, btnId, url, grid) {
    $(e).button("loading");
    window.location.href = [url, moudleId, menuId, btnId].join("/");
}

//editModel
InBed.editModel = function (e, moudleId, menuId, btnId, url, grid) {
...
//delete
InBed.delModel = function (e, moudleId, menuId, btnId, url, grid) {
...
//authen
InBed.authen = function (e, moudleId, menuId, btnId, url, grid) {
    var editRow = grid.getSelectedRow();
    if (editRow && editRow.Id) {
        $(e).button("loading");
        window.location.href = [url, moudleId, menuId, btnId, editRow.Id].join("/");
...
InBed.buttonLoading = function(form) {
