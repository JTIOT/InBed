...
//加载datatable
InBed.grid.prototype.load = function() {
    ...
    this.columns[0].data = function(e) {
        return '<label class="checkbox-custom inline check-success" style="margin:0"><input type="checkbox" value=" " name="checkbox" id="checkbox-' + e.Id + '"/><label for="checkbox-' + e.Id + '" style="margin:3px 0 0 0">&nbsp;</label></label>';
    };
    ...
    _grid.DataTable({
        ...
        ajax: {
            ...
            data: function(d) { //d 是原始的发送给服务器的数据，默认很长。
                ...
                var formData = $("#fromSearch").serializeArray(); //把form里面的数据序列化成数组
                formData.forEach(function (e) {
                    searchData[e.name] = e.value;
                });
                var formData1 = $("#fromSearch1").serializeArray(); //把form里面的数据序列化成数组
                formData1.forEach(function (e) {
                    searchData[e.name] = e.value;
                });
        ...
        initComplete: function (setting, json) {
            ...
            //console.log("initComplete-json:" + json.toString());
            {
                ...
                $(tablePrefix + "filter").find("button[name='search']").click(function() {
            ...
            $(this).find("tbody tr").each(function () {
                $(this).click(function () {
                    ...
                    if ($(this).hasClass("selected")) {
                        ...
                        $(this).removeClass("checked").removeClass("checked");
...
InBed.grid.prototype.getSelectedRow = function () {
    var _this = this;
    var _grid = $("#" + _this.id);
...
//获取datatable所有选中行的数据
InBed.grid.prototype.getSelectedRows = function () {
    var _this = this;
    var _grid = $("#" + _this.id);
    var selectedRows = [];
...
//重新加载
InBed.grid.prototype.reload = function () {
    $("#" + this.id).DataTable().draw();
    $('.recordName .tag').each(function () {
        var id = this.id;
        var oDom = $('#checkbox-' + id).parent().parent();
...
//获取datatable选中行的最后一条数据
InBed.grid.prototype.getCheckRow = function () {
    var _this = this;
    var _grid = $("#" + _this.id);
    var selectedRow = null;
    _grid.find("tbody tr.checked:last").each(function () {
        selectedRow = _grid.DataTable().row($(this)).data();
    });
    return selectedRow;
}
...
//获取datatable取消行的数据
InBed.grid.prototype.getCancelRow = function () {
    var _this = this;
    var _grid = $("#" + _this.id);
    var selectedRow = null;
    _grid.find("tbody tr.unselected").each(function () {
        selectedRow = _grid.DataTable().row($(this)).data();
    });
    return selectedRow;
}
