...
this IDbConnection cnn, string sql, Func<TFirst, TSecond, TThird, TFourth, TReturn> map, object param = null,
        ...
        private static void PassByPosition(IDbCommand cmd)
        {
            ...
            var parameters = new Dictionary<string, IDbDataParameter>(StringComparer.InvariantCulture);

            foreach (IDbDataParameter param in cmd.Parameters)
            {
                if (!string.IsNullOrEmpty(param.ParameterName)) parameters[param.ParameterName] = param;
            }
            var consumed = new HashSet<string>(StringComparer.InvariantCulture);
            var firstMatch = true;
...
#pragma warning restore 618
        ...
        private static readonly Dictionary<TypeCode, MethodInfo> toStrings = new[]
        {
            ...
            typeof (decimal)
        }.ToDictionary(Type.GetTypeCode, x => x.GetMethod("ToString", BindingFlags.Public | BindingFlags.Instance, null, new[] { typeof(IFormatProvider) }, null));
...
#pragma warning restore 618
    ...
    /// </summary>
            ...
            private IEnumerable<T> ReadImpl<T>(Type type, bool buffered)
            {
                ...
                var cache = GetCacheInfo(typedIdentity, null, true);
